...
pub fn read_metadata(vec: &[u8]) -> Vec<f64> {
    ...
    &mut std::io::BufReader::new(vec)).unwrap();
    ...
    match reader.get_field(exif::Tag::GPSLatitude, false).unwrap().value {
    ...
    match reader.get_field(exif::Tag::GPSLongitude, false).unwrap().value {
    ...
    match reader.get_field(exif::Tag::GPSLatitudeRef, false).unwrap().value {
    ...
    match reader.get_field(exif::Tag::GPSLongitudeRef, false).unwrap().value {
        Value::Ascii(ref referencia) => {
